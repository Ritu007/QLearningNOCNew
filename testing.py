...
q_table_fle =  'qtable_grid5x5_vcs2_1767004231.pkl'
loaded_array = np.load(q_table_fle, allow_pickle=True)
    ...
    if show:
        ...
        for i in range(SIZE):
            for j in range(SIZE):
                ...
                #
                # if i % 2 == 0 and j > 0 and i < SIZE - 1:
                #     cv2.line(env, (x, y), (x - NODE_SIZE, y + NODE_SIZE), ORANGE, 2)
                # if i % 2 != 0 and j < SIZE - 1:
                #     cv2.line(env, (x, y), (x + NODE_SIZE, y + NODE_SIZE), ORANGE, 2)

                # Draw diagonal Z-pattern links
...
#                     restrict = -1
#                     if (current_node.x, current_node.y - 1) in faulty_routers:
#                         restrict = 0
#                     if (current_node.x + 1, current_node.y) in faulty_routers:
#                         restrict = 1
#                     if (current_node.x, current_node.y + 1) in faulty_routers:
#                         restrict = 2
#                     if (current_node.x - 1, current_node.y) in faulty_routers:
#                         restrict = 3
#                     if restrict == max_choice:
#                         _, second_max = get_second_max(choice_rank)
